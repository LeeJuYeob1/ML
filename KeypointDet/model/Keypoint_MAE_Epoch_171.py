...
model.compile(optimizer='adam', 
              ...
              metrics=['mae'])
# 7. 모델 학습
history = model.fit(X_train,y_train,epochs = 171,batch_size = 512, shuffle=False, validation_split=0.2)
...
import cv2
test = X_test[100].reshape(1,96,96,1)
output = model.predict(test)

x_list = []
y_list = []
for num, i in enumerate(output[0]):
    if num%2==0:
        x_list.append(i)
    else:
        y_list.append(i)
plt.imshow(X_test[100].reshape(96,96),cmap='gray')
plt.scatter(x_list,y_list)
plt.show()

test = X_test[200].reshape(1,96,96,1)
output = model.predict(test)
...
for num, i in enumerate(output[0]):
    if num%2==0:
        ...
        y_list.append(i)
plt.imshow(X_test[200].reshape(96,96),cmap='gray')
plt.scatter(x_list,y_list)
plt.show()