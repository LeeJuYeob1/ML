...
"""
path = "testsets/RealSRSet/videoplayback.mp4"
save_path = "testsets/video/"
vidcap = cv2.VideoCapture(path)
success,image = vidcap.read()
count = 0
while success:
  cv2.imwrite(save_path + "/%06d.jpg" % count, image)     # save frame as JPEG file
  success,image = vidcap.read()
...
#    print(torch.backends.cudnn.version())  # cudnn version
    ...
    testset_Ls = ['VIDEO']  # ['RealSRSet','DPED']
...
#            model.load_state_dict(state_dict, strict=True)
        ...
        for testset_L in testset_Ls:
            ...
            idx = 0
            path = "testsets/RealSRSet/videoplayback.mp4"
            for img in util._get_paths_from_images(L_path):
                # --------------------------------
                # (1) img_L
                # --------------------------------
                idx += 1
